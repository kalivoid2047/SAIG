...
- **Phase 3 — Predictive Core:** ✅ Complete — yield prediction, demand forecasting, six-domain risk intelligence, OR-Tools capacitated route optimization, and model evaluation (out-of-sample demand backtest + yield predicted-vs-actual). In-process ML plane ([ADR-0003](../adr/0003-in-process-ml-plane.md)): sklearn gradient-boosted trees + model registry with lineage, training CLI. 97 backend tests green.
- **Phase 4 — AI Layer:** ⏭️ Next — recommendations, copilot (RAG), documents, scenarios.
